...
Sub B1B2Step1()
    ...
    For i = 2 To lastRow
        If Not ws.Rows(i).Hidden Then
            ...
            ws.Cells(i, colNumFromCell).Formula = "=XLOOKUP(B" & i & ",Report!$H$2:$H$20000,Report!$I2:$I20000,,,1)"  ' <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            ws.Cells(i, 23).Formula = "=IF(ISERROR(XLOOKUP(B" & i & ",Report!$S$2:$S$1000,Report!$S$2:$S$1000,,,1))=TRUE,NA(),""Defer"")"
            ws.Cells(i, 116).Formula = "=XLOOKUP(B" & i & ",Report!$P$2:$P$1000,Report!$P$2:$P$1000,,,1)"
        End If
    Next i
    ...
    ' Convert formulas to values
    Set formulaRange = ws.Range("DL2:DL" & lastRow)
    If Not formulaRange Is Nothing Then
        For Each lookupCell In formulaRange
            If lookupCell.HasFormula Then
                lookupCell.Value = lookupCell.Value
    ...
    For i = 2 To lastRow
        If Not ws.Rows(i).Hidden Then
            ...
            ws.Cells(i, 19).Formula = "=IF(AND(OR(M" & i & "=1,M" & i & "=""""),OR(F" & i & "=2,Q" & i & ">=2)),1,"""")"
            ws.Cells(i, 20).Formula = "=IF(AND(OR(M" & i & "=1,M" & i & "=""""),F" & i & "=2,Q" & i & ">=3),1,"""")"
